...
export function DashboardPage() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <section className="flex min-h-[40vh] items-center justify-center text-sm text-slate-500">Loading your finances...</section>
      </AppShell>
    );
  }
  ...
  if (error) {
    return (
      <AppShell>
        <section className="rounded-3xl border border-rose-500/30 bg-rose-500/10 p-6 text-sm text-rose-700">{error}</section>
      </AppShell>
    );
  }
      ...
      <section className="space-y-8">
        <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-emerald-50 via-white to-slate-50 p-6">
            <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Current balance</p>
            <div className="mt-4 flex items-end justify-between gap-4">
              <div>
                <h2 className="text-4xl font-semibold text-slate-900">{formatCurrency(currentBalance)}</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <MetricCard icon={ArrowUpRight} label="Income this month" value={formatCurrency(monthlyIncome, 0, 0)} tone="text-emerald-600" />
            <MetricCard icon={ArrowDownRight} label="Expenses this month" value={formatCurrency(monthlyExpenses, 0, 0)} tone="text-rose-600" />
          </div>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Budget overview</p>
                <h3 className="mt-1 text-xl font-semibold text-slate-900">Monthly progress</h3>
              </div>
            ...
            <div className="mt-5 space-y-4">
              {budgetSummary.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No budgets yet.{' '}
                  <Link to="/budgets" className="text-emerald-700 hover:underline">
                    Create a budget
                  </Link>
                </p>
              ) : (
                budgetSummary.map((budget) => (
                  <div key={budget.label} className="space-y-2">
                    ...
                    <div className="h-2 rounded-full bg-slate-100">
                      <div className="h-2 rounded-full bg-gradient-to-r from-accent to-emerald-500" style={{ width: `${budget.percentUsed}%` }} />
                    </div>
                  </div>
                ))
              )}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5">
            ...
            <h3 className="mt-1 text-xl font-semibold text-slate-900">Latest transactions</h3>
              ...
              {recentTransactions.length === 0 ? (
                ...
                recentTransactions.map((transaction) => (
                  ...
                  >
                    ...
                    <p className={transaction.transactionType === 'expense' ? 'text-rose-600' : 'text-emerald-600'}>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Savings goals</p>
              <h3 className="mt-1 text-xl font-semibold text-slate-900">Goal progress</h3>
            </div>
            <Link to="/goals" className="text-sm text-emerald-700 hover:underline">
              Manage goals
            </Link>
          </div>
          <div className="mt-5 space-y-4">
            {goals.length === 0 ? (
              <p className="text-sm text-slate-500">
                No goals yet.{' '}
                <Link to="/goals" className="text-emerald-700 hover:underline">
